...
const Projects = () => {
    return (
        <div className="bg-gray-50 p-6 text-center mx-0 md:mx-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mx-0 md:mx-20">
                {projects.map((project) => (
                    <Link
                        key={project.id}
                        to={`/project/${project.title}`}
                        state={{ project }}
                        className="relative bg-cover bg-center h-80 flex flex-col  items-center text-black p-4 rounded-lg shadow-md"
                    >
